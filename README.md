...
Identity is cryptographic and created on your device when you set a display name. There is no global people search: Inertia assumes friends already know each other and already have a way to share a signed invite link (or QR code) outside the app. A thin [VPS `inertia-relay`](crates/inertia-relay/README.md) helps peers reach each other through NAT. After that, **DCUtR hole punching** tries to open a direct channel so large payloads (images, videos, and shared files) are rerouted peer-to-peer and avoid the relay when possible, which keeps relay bandwidth and hosting cost minimal. Content stays end-to-end encrypted; the relay only forwards encrypted circuits and never holds data.
