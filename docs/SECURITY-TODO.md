...
- [ ] **API auth on non-localhost** - required for mobile shells; token or IPC boundary so other processes cannot drive `127.0.0.1:4783`. **Android on-device install** already binds API to loopback only; auth still needed before any LAN/0.0.0.0 exposure.
